    ...
    *,
    ...
    if sandbox_config is not None and sandbox_config.is_active:
        ...
        if sandbox_config.backend == "docker":
            ...
            _log.info("🛡️ Cloud sandbox backend created: %s health=%s", type(sandbox_backend).__name__, sandbox_backend.health_check())
            if not sandbox_backend.health_check():
        ...
        _log.info("🛡️ Sandbox active: mode=%s backend=%s backend_class=%s",
        ...
        _log.info("🛡️ Sandbox NOT active: config=%s is_active=%s",
